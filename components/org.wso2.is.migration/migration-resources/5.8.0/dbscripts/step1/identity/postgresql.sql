...
DO $$ BEGIN BEGIN ALTER TABLE IDN_OAUTH2_AUTHORIZATION_CODE ADD COLUMN IDP_ID INTEGER NOT NULL DEFAULT -1; EXCEPTION WHEN duplicate_column THEN RAISE NOTICE 'column IDP_ID already exists in IDN_OAUTH2_AUTHORIZATION_CODE.'; END;	BEGIN ALTER TABLE IDN_OAUTH2_ACCESS_TOKEN ADD COLUMN IDP_ID INTEGER NOT NULL DEFAULT -1; EXCEPTION WHEN duplicate_column THEN RAISE NOTICE 'column IDP_ID already exists in IDN_OAUTH2_ACCESS_TOKEN.'; END;	BEGIN ALTER TABLE IDN_OAUTH2_ACCESS_TOKEN_AUDIT ADD COLUMN IDP_ID INTEGER NOT NULL DEFAULT -1; EXCEPTION WHEN duplicate_column THEN RAISE NOTICE 'column IDP_ID already exists in IDN_OAUTH2_ACCESS_TOKEN_AUDIT.'; END; END$$;
